...
export default {
  ...
  'component.pluginForm.limit-count.redis_username.tooltip': '当使用 redis 限速策略时，该属性是 Redis 服务节点的用户名。',
  'component.pluginForm.limit-count.redis_ssl.tooltip': '当使用 redis 限速策略时，是否启用 SSL。',
  'component.pluginForm.limit-count.redis_ssl_verify.tooltip': '当使用 redis 限速策略时，是否验证 SSL 证书。',
  'component.pluginForm.limit-count.redis_keepalive_timeout.tooltip': '当使用 redis 限速策略时，Redis 连接的空闲超时时间（毫秒）。',
  'component.pluginForm.limit-count.redis_keepalive_pool.tooltip': '当使用 redis 限速策略时，Redis 连接池的大小。',
