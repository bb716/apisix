...
const LimitConn: React.FC<Props> = ({ form, schema }) => {
  ...
  return (
    <Form form={form} {...FORM_ITEM_LAYOUT}>
      ...
      >
        <InputNumber min={properties.conn?.exclusiveMinimum || 0} required />
      </Form.Item>
      <Form.Item
        label="burst"
        required
      ...
      >
        <InputNumber step={0.001} min={properties.default_conn_delay?.exclusiveMinimum || 0} required />
      </Form.Item>
      <Form.Item
        label="only_use_default_delay"
        name="only_use_default_delay"
        valuePropName="checked"
        initialValue={properties.only_use_default_delay?.default || false}
        tooltip={formatMessage({
