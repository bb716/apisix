...
func (h *SchemaHandler) PluginSchema(c droplet.Context) (interface{}, error) {
	...
	// If plugin is enabled but not in schema json, return a generic object schema so frontend still works
	if ret == nil {
		if enabled, ok := conf.Plugins[input.Name]; ok && enabled {
			ret = map[string]interface{}{"type": "object"}
		}
	}
